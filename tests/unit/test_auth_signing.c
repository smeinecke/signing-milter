...
static void test_authorized_single_signer(void** state) {
    ...
    assert_true(create_auth_cdb(path,
                                "alice@example.org", "sender@example.com",
                                NULL) == 0);
...
static void test_multiple_signers_per_auth_identity(void** state) {
    ...
    assert_true(create_auth_cdb(path,
                                "alice@example.org", "sender@example.com",
                                "alice@example.org", "sales@example.org",
                                NULL) == 0);
    ...
    assert_int_equal(auth_signing_authorized("alice@example.org", "sender@example.com"), 1);
    assert_int_equal(auth_signing_authorized("alice@example.org", "sales@example.org"), 1);
    assert_int_equal(auth_signing_authorized("alice@example.org", "other@example.org"), 0);

    dict_close(&dict_auth_signingtable);
    unlink(path);
}

static void test_list_value_and_case_bracket_normalization(void** state) {

    (void) state;
    char path[] = "/tmp/signing-milter-test-auth-XXXXXX";

    assert_true(create_auth_cdb(path,
                                "alice@example.org", "Sender@EXAMPLE.COM, sales@example.org",
                                "bob@example.org", "<Sender@Example.COM>",
                                NULL) == 0);
    ...
    assert_int_equal(auth_signing_authorized("ALICE@EXAMPLE.ORG", "<sender@example.com>"), 1);
    assert_int_equal(auth_signing_authorized("<alice@example.org>", "SALES@EXAMPLE.ORG"), 1);
    assert_int_equal(auth_signing_authorized("bob@example.org", "sender@example.com"), 1);
    assert_int_equal(auth_signing_authorized("bob@example.org", "other@example.com"), 0);
...
static void test_unauthorized_auth_identity(void** state) {
    ...
    assert_int_equal(auth_signing_authorized("bob@example.org", "sender@example.com"), 0);
    /* mallory is not present at all */
    assert_int_equal(auth_signing_authorized("mallory@example.org", "sender@example.com"), 0);
...
int main(void) {
    ...
    const struct CMUnitTest tests[] = {
        ...
        cmocka_unit_test(test_list_value_and_case_bracket_normalization),
